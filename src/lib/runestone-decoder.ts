import { SeekBuffer } from '../utils/seekbuffer';
 ...
 */
export function decodeVarInt(seekBuffer: SeekBuffer): bigint | null {
 ...
 */
export function readTagValues(seekBuffer: SeekBuffer): bigint[] {
  const values: bigint[] = [];
 ...
 * @param seekBuffer Buffer to read from
 */
export function skipTagValues(seekBuffer: SeekBuffer): void {
 ...
 */
export function decipherRunestone(buffer: Buffer): any {
  ...
  try {
    ...
    const seekBuffer = new SeekBuffer(data);
 ...
 */
export function decodeOpReturn(opReturnHex: string): any {
    ...
    if (hex.startsWith('6a')) {
      ...
      for (const data of pushedData) {
        ...
        if (data.length >= 2 && data[0] === 0xff && data[1] === 0x7f) {
          ...
          const seekBuffer = new SeekBuffer(data.slice(2));
