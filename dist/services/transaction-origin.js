...
exports.getOutpointInfo = getOutpointInfo;
exports.getTransactionInputs = getTransactionInputs;
exports.isContractCreation = isContractCreation;
exports.isFactoryDeployment = isFactoryDeployment;
exports.traceTransactionOrigin = traceTransactionOrigin;
exports.getExecutionContext = getExecutionContext;
const metashrew_api_1 = require("./metashrew-api");
const alkanes_trace_1 = require("./alkanes-trace");
/**
 ...
 */
async function getOutpointInfo(txid, vout) {
    try {
        ...
        return await (0, metashrew_api_1.callView)('protorunesbyoutpoint', hexInput);
    }
    catch (error) {
 ...
 */
async function getTransactionInputs(txid, rawTx) {
    try {
        ...
        const inputsWithInfo = await Promise.all(inputs.map(async (input) => {
 ...
 */
function isContractCreation(outpointInfo) {
    ...
    return runestones.some((stone) => stone.op === 'new-token' ||
        stone.op === 'deploy' ||
 ...
 */
function isFactoryDeployment(outpointInfo) {
    ...
    // Simplified example:
    const runestones = outpointInfo.runestones || [];
    return runestones.some((stone) => stone.op === 'factory-deploy' ||
 ...
 */
async function traceTransactionOrigin(txid, vout) {
    try {
        console.log(`Tracing origin for transaction ${txid}:${vout}`);
        // Get the execution trace for this transaction
        const trace = await (0, alkanes_trace_1.getTransactionTrace)(txid, vout);
        if (!trace) {
 ...
 */
async function getExecutionContext(txid, vout) {
    try {
        // Get the trace data
        const trace = await (0, alkanes_trace_1.getTransactionTrace)(txid, vout);
...
//# sourceMappingURL=transaction-origin.js.map